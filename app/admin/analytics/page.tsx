...
/** Most-recent rows rendered in the readable table; the rest live in the raw-data dump below. */
const TABLE_ROW_CAP = 30;
...
export default async function AdminAnalyticsPage({
  ...
  let timeSeries: DailyTimeSeriesRow[] = [];
  let error: string | null = null;
  try {
    [summary, events, timeSeries] = await Promise.all([
      getSummary(filters),
      getEvents(filters),
      getDailyTimeSeries(filters),
    ]);
  } catch {
    error =
      ...
      <div className="mx-auto max-w-[1024px]">
        ...
        {error ? (
          ...
          summary && (
            <>
              {/* ── Trend charts ──────────────────────────────────────── */}
              <h2 className="mt-8 font-display text-[18px] font-bold text-ink">Trends</h2>
              <div className="mt-3 grid grid-cols-1 gap-3 lg:grid-cols-2">
                <UniqueUsersChart data={timeSeries} />
                <CompletionRateChart data={timeSeries} />
                <HintsByTypeChart hintsByKey={summary.hintsByKey} />
                <SharesByButtonChart data={timeSeries} />
              </div>

              {/* ── Raw event table (capped to the 30 most recent) ───── */}
              <h2 className="mt-8 font-display text-[18px] font-bold text-ink">
                Raw events{" "}
                <span className="font-bold text-sm text-muted">
                  (showing {Math.min(events.length, TABLE_ROW_CAP)} of {events.length})
                </span>
              </h2>
                ...
                <table className="w-full border-collapse text-left text-sm">
                  ...
                  <tbody>
                    {events.length === 0 ? (
                      ...
                      events.slice(0, TABLE_ROW_CAP).map((row) => (
